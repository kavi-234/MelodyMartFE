    ...
    <div class="mm-sidebar-inner">
      ...
      <div v-if="!collapsed" class="mm-sidebar-profile">
        ...
        <div class="profile-info">
          ...
          <div class="profile-role" style="color: var(--mm-coral)">Admin</div>
        </div>
      </div>
      <div v-else class="flex justify-center py-3">
        <div class="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold" style="background: linear-gradient(135deg,#E07060,#C04A3A); color: #fff">
...
const items = [
  { label: 'Dashboard',   path: '/dashboard/admin',             icon: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>` },
  { label: 'Users',       path: '/dashboard/admin/users',       icon: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>` },
  { label: 'Orders',      path: '/dashboard/admin/orders',      icon: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 002 1.61h9.72a2 2 0 002-1.61L23 6H6"/></svg>` },
  { label: 'Instruments', path: '/dashboard/admin/instruments', icon: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>` },
...
const isActive = (path) => { if (path === '/dashboard/admin') return route.path === '/dashboard/admin'; return route.path.startsWith(path) }
const onMenuClick = (item) => { if (item.path === '/dashboard/admin') emits('compress'); else emits('expand'); closeOnMobile() }
...
.mm-nav-item.is-active { background: rgba(224,112,96,0.08); color: #EE9080; border: 1px solid rgba(224,112,96,0.15); }
.mm-nav-item.is-active::before { content: ''; position: absolute; left: 0; top: 20%; bottom: 20%; width: 2.5px; background: var(--mm-coral); border-radius: 0 2px 2px 0; }
...
.profile-avatar-fallback { width: 34px; height: 34px; border-radius: 50%; background: linear-gradient(135deg, #E07060, #C04A3A); display: flex; align-items: center; justify-content: center; font-size: 0.85rem; font-weight: 700; color: #fff; flex-shrink: 0; }
...
.profile-role { font-size: 0.68rem; font-weight: 500; margin-top: 0.05rem; }
...
.mm-mobile-toggle:hover { color: var(--mm-coral); }
